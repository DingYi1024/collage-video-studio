...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    try:
        return editorial_runtime.compile_composition(value)
    except editorial_runtime.EditorialError as exc:
...
def validate_manifest(path: Path) -> tuple[list[str], list[str], dict[str, int]]:
    ...
    for index, layer in enumerate(layers, 1):
        ...
        primitive = layer.get("primitive")
        if primitive is None:
            source = path.parent / str(layer.get("path", ""))
            if not source.is_file() or source.stat().st_size <= 0:
                errors.append(f"{layer_id or index}: missing layer image {source}")
        elif not isinstance(primitive, dict):
            errors.append(f"{layer_id or index}: primitive must be an object")
        else:
            ...
            if primitive_kind != "group":
                try:
                    ...
                    py = float(primitive.get("y", 0))
                    pw = float(primitive.get("width", canvas.get("width", 0)))
                    ph = float(primitive.get("height", canvas.get("height", 0)))
                    if pw <= 0 or ph <= 0:
                        raise ValueError
                    if (
                        px < 0 or py < 0
                        or px + pw > float(canvas.get("width", 0))
                        or py + ph > float(canvas.get("height", 0))
                    ):
                        errors.append(
                            f"{layer_id or index}: primitive bounds exceed canvas"
                        )
                except (TypeError, ValueError):
                    errors.append(
                        f"{layer_id or index}: primitive bounds must be positive numbers"
                    )
            if primitive_kind == "text":
                try:
                    fits, chosen_size = primitive_text_fits(
                        primitive,
                        (
                    ...
                    if not fits:
                        errors.append(
                            f"{layer_id or index}: text overflows at min_font_size "
                            f"{chosen_size}"
                        )
                except (TypeError, ValueError):
                    errors.append(
                        f"{layer_id or index}: invalid text primitive typography"
                    )
        pose_sequence = layer.get("pose_sequence")
...
def _color(value: Any, fallback: str = "#ffffff") -> Any:
    return value if value not in (None, "") else fallback


def primitive_text_fits(
    primitive: dict[str, Any],
    canvas: tuple[int, int],
) -> tuple[bool, int]:
    """Return whether text fits its box and the selected font size."""
    ...
    padding = max(0, round(float(primitive.get("padding", 0))))
    requested = max(1, round(float(primitive.get("font_size", 64))))
    minimum = max(1, round(float(primitive.get("min_font_size", 16))))
    if minimum > requested:
        raise ValueError("min_font_size exceeds font_size")
    font_path = primitive.get("font_path")
    bold = bool(primitive.get("bold", False))
    available_width = max(1, width - padding * 2)
    available_height = max(1, height - padding * 2)
    for size in range(requested, minimum - 1, -1):
        candidate = _font(size, bold, str(font_path) if font_path else None)
        spacing = round(float(primitive.get("line_spacing", size * 0.22)))
        bounds = draw.multiline_textbbox(
            (0, 0), text, font=candidate, spacing=spacing
        )
        if (
...
def render_primitive(
    ...
    if kind == "rectangle":
        draw.rounded_rectangle(
            box,
            radius=max(0, round(float(primitive.get("radius", 0)))),
            fill=_color(primitive.get("fill"), "#ffffff"),
            outline=primitive.get("stroke"),
            width=max(1, round(float(primitive.get("stroke_width", 1)))),
        )
    elif kind == "ellipse":
        draw.ellipse(
            box,
        ...
        points = primitive.get("points", [[x, y], [x + width, y + height]])
        draw.line(
            [tuple(map(float, point)) for point in points],
            fill=_color(primitive.get("color"), "#ffffff"),
            width=max(1, round(float(primitive.get("stroke_width", 4)))),
            joint="curve",
        )
    elif kind == "text":
        padding = max(0, round(float(primitive.get("padding", 0))))
        background = primitive.get("background")
        if background:
            draw.rounded_rectangle(
                box,
                radius=max(0, round(float(primitive.get("radius", 0)))),
        ...
        font_path = primitive.get("font_path")
        bold = bool(primitive.get("bold", False))
        fits, chosen_size = primitive_text_fits(primitive, canvas)
        chosen = _font(chosen_size, bold, str(font_path) if font_path else None)
        available_width = max(1, width - padding * 2)
        available_height = max(1, height - padding * 2)
        _ = fits, requested, minimum, available_width, available_height
        spacing = round(float(primitive.get("line_spacing", chosen_size * 0.22)))
        bounds = draw.multiline_textbbox((0, 0), text, font=chosen, spacing=spacing)
        text_width = bounds[2] - bounds[0]
        text_height = bounds[3] - bounds[1]
        align = str(primitive.get("align", "left"))
        valign = str(primitive.get("valign", "top"))
        text_x = x + padding
        text_y = y + padding
        if align == "center":
        ...
        values = [float(value) for value in primitive.get("values", [])]
        if values:
            maximum = max(1e-9, max(values))
            gap = float(primitive.get("gap", 10))
            bar_width = max(1.0, (width - gap * (len(values) - 1)) / len(values))
            colors = primitive.get("colors", ["#f2c14e", "#3d6b73", "#d94b3d"])
            for index, value in enumerate(values):
                ...
                draw.rounded_rectangle(
                    (
                        round(left),
                        round(y + height - bar_height),
                        round(left + bar_width),
                        round(y + height),
                    ),
                    radius=max(0, round(float(primitive.get("radius", 4)))),
        ...
        draw.line(
            ...
            fill=_color(primitive.get("color"), "#f2c14e"),
            width=max(1, round(float(primitive.get("stroke_width", 4)))),
        )
        font = _font(round(float(primitive.get("font_size", 22))), True)
        for index, item in enumerate(items):
            ratio = float(item.get("position", index / max(1, len(items) - 1)))
            px = x + ratio * width
            radius = float(primitive.get("marker_radius", 8))
            draw.ellipse((px - radius, line_y - radius, px + radius, line_y + radius),
                         fill=_color(item.get("color"), "#ffffff"))
            label = str(item.get("label", ""))
            label_box = draw.textbbox((0, 0), label, font=font)
            label_width = label_box[2] - label_box[0]
            label_x = min(
                canvas[0] - label_width - 4,
                max(4, px - label_width / 2),
            )
            draw.text(
                (label_x, line_y - 30),
                label,
                font=font,
                fill=_color(primitive.get("text_color"), "#ffffff"),
            )
    elif kind == "annotation":
        target = primitive.get("target", [x, y])
        label = primitive.get("label_box", [x + 40, y + 40, width, height])
        lx, ly, lw, lh = map(float, label)
        draw.line((float(target[0]), float(target[1]), lx, ly + lh / 2),
                  fill=_color(primitive.get("color"), "#f2c14e"),
                  width=max(1, round(float(primitive.get("stroke_width", 3)))))
        draw.rounded_rectangle((lx, ly, lx + lw, ly + lh), radius=6,
                               fill=_color(primitive.get("background"), "#20242b"))
        draw.text((lx + 10, ly + 8), str(primitive.get("text", "")),
                  font=_font(round(float(primitive.get("font_size", 22))), True),
                  fill=_color(primitive.get("text_color"), "#ffffff"))
    elif kind == "map-route":
        points = [tuple(map(float, point)) for point in primitive.get("points", [])]
        if len(points) >= 2:
            draw.line(points, fill=_color(primitive.get("color"), "#d94b3d"),
                      width=max(1, round(float(primitive.get("stroke_width", 6)))),
                      joint="curve")
            radius = float(primitive.get("marker_radius", 7))
            for px, py in points:
...
def load_layer_sources(
    ...
    for layer in sorted(manifest["layers"], key=lambda item: float(item.get("z", 0))):
        paths = {str(layer["path"])} if layer.get("path") else set()
...
def sprite_at(
    ...
    sprites = layer.get("sprites", [])
    if not sprites:
        if "__primitive__" in sources:
            return sources["__primitive__"], layer
